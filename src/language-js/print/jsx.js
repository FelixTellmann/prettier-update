...
function printJsxElementInternal(path, options, print) {
  ...
  if (
    ...
    && (node.children[0].type === "JSXExpressionContainer"
      && (node.children[0].expression.type === "TemplateLiteral" || node.children[0].expression.type === "TaggedTemplateExpression"))
    || (node.children.length === 1 && node.children[0].type === "JSXElement" && node.children[0].children.length === 1 && node.loc.start.line === node.loc.end.line)
  ) {
