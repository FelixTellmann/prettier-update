...
function printMemberChain(path, options, print) {
  ...
  if (
    ...
    (callExpressions.length > 2 && callExpressions.some((expr) => !expr.arguments.every((arg) => isSimpleCallArgument(arg, 0)))) ||
    printedGroups.slice(0, -1).some(willBreak) ||
    lastGroupWillBreakAndOtherCallsHaveFunctionArguments() ||
    (options.breakLongMethodChains && willBreak(oneLine)) ||
    (options.breakLongMethodChains && shouldHaveEmptyLineBeforeIndent)
  ) {
